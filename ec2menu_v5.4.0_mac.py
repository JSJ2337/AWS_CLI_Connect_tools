...
# -*- coding: utf-8 -*-
"""
EC2, RDS, ElastiCache, ECS, EKS 접속 자동화 스크립트 v5.4.0 (macOS 전용)
...
class PerformanceCache:
    ...
    def _get_ttl_for_key(self, key: str) -> int:
        """캐시 키에서 리소스 타입을 추출하여 적절한 TTL 반환 (v5.4.0)"""
...
# ----------------------------------------------------------------------------
# macOS Keychain 자격 증명 관리 (v5.4.0 신규)
# ----------------------------------------------------------------------------
...
class InteractiveMenu:
    ...
    def __init__(
        ...
        """
        Args:
            items: 표시할 항목 리스트 (이미 포맷된 문자열)
            title: 메뉴 상단 제목
            footer: 메뉴 하단 도움말
            selected_idx: 초기 선택 인덱스
            show_index: 항목 앞에 번호 표시 여부
            page_size: 페이지당 항목 수 (0이면 터미널 높이에 맞춤)
        """
        self.items = items
        self.title = title
        self.footer = footer if footer else "↑↓: 이동  Enter: 선택  q: 취소  /: 검색"
        self.selected_idx = max(0, min(selected_idx, len(items) - 1)) if items else 0
        self.show_index = show_index
        self.page_size = page_size
        self.search_mode = False
        self.search_query = ""
        self.filtered_indices: List[int] = list(range(len(items)))

    def _get_display_range(self, max_height: int) -> Tuple[int, int]:
        """현재 선택 항목이 보이도록 표시 범위 계산"""
        if not self.filtered_indices:
            return 0, 0

        # 필터된 인덱스 중 현재 선택된 위치
        try:
            current_pos = self.filtered_indices.index(self.selected_idx)
        except ValueError:
            current_pos = 0
            if self.filtered_indices:
                self.selected_idx = self.filtered_indices[0]

        display_count = self.page_size if self.page_size > 0 else max(max_height - 6, 5)

        # 선택 항목이 화면 중앙에 오도록 조정
        start = max(0, current_pos - display_count // 2)
        end = min(len(self.filtered_indices), start + display_count)

        # 시작 위치 재조정 (끝에 도달했을 때)
        if end - start < display_count:
            start = max(0, end - display_count)
    ...
    def _filter_items(self) -> None:
        ...
        # 현재 선택이 필터 결과에 없으면 첫 번째 항목 선택
        if self.filtered_indices and self.selected_idx not in self.filtered_indices:
            self.selected_idx = self.filtered_indices[0]
    ...
    def _draw_menu(self, stdscr) -> None:
        ...
        curses.init_pair(1, curses.COLOR_BLACK, curses.COLOR_WHITE)  # 선택됨
        curses.init_pair(2, curses.COLOR_CYAN, -1)  # 제목
        curses.init_pair(3, curses.COLOR_YELLOW, -1)  # 도움말
        curses.init_pair(4, curses.COLOR_GREEN, -1)  # 검색
        ...
        if self.title:
            ...
            if len(title_text) < width:
                ...
                stdscr.addstr(current_row, 0, title_text[:width-1])
                stdscr.attroff(curses.color_pair(2) | curses.A_BOLD)
            current_row += 1
            stdscr.addstr(current_row, 0, "─" * min(width - 1, 60))
            current_row += 1

        # 검색 모드 표시
        if self.search_mode or self.search_query:
            search_text = f"🔍 검색: {self.search_query}_" if self.search_mode else f"🔍 필터: {self.search_query} (ESC: 초기화)"
            stdscr.attron(curses.color_pair(4))
            stdscr.addstr(current_row, 0, search_text[:width-1])
            stdscr.attroff(curses.color_pair(4))
            current_row += 1

        # 항목 표시 범위 계산
        available_height = height - current_row - 3  # 하단 여백
        start, end = self._get_display_range(available_height)

        # 스크롤 표시 (상단)
        if start > 0:
            stdscr.addstr(current_row, 0, f"  ↑ ({start} more above)")
            current_row += 1

        # 항목 표시
        for display_pos in range(start, end):
            if current_row >= height - 2:
                break

            real_idx = self.filtered_indices[display_pos]
            item_text = self.items[real_idx]

            # 인덱스 표시
            if self.show_index:
                prefix = f"{real_idx + 1:3d}) "
            else:
                prefix = "  "

            # 선택된 항목 하이라이트
            is_selected = (real_idx == self.selected_idx)
            ...
            line = f"{marker}{prefix}{item_text}"
            # 화면 너비에 맞게 자르기
            if len(line) > width - 1:
                line = line[:width - 4] + "..."

            try:
                stdscr.addstr(current_row, 0, line)
            except curses.error:
                pass
        ...
        if end < len(self.filtered_indices):
            remaining = len(self.filtered_indices) - end
            try:
                stdscr.addstr(current_row, 0, f"  ↓ ({remaining} more below)")
            except curses.error:
                pass
            current_row += 1
        ...
        # 하단 도움말
        footer_row = height - 1
        if footer_row > current_row:
            stdscr.attron(curses.color_pair(3))
            try:
                stdscr.addstr(footer_row, 0, self.footer[:width-1])
            except curses.error:
                pass
    ...
    def _handle_input(self, stdscr) -> int:
        """키 입력 처리, 선택된 인덱스 반환 (취소 시 -1)"""
        while True:
            ...
            if self.search_mode:
                ...
                if key == curses.KEY_UP or key == ord('k'):
                    ...
                    if self.filtered_indices:
                        self.selected_idx = self.filtered_indices[-1]
                elif key == 10 or key == curses.KEY_ENTER:  # Enter
                    if self.filtered_indices:
    ...
    def _move_selection(self, delta: int) -> None:
        ...
        new_pos = max(0, min(len(self.filtered_indices) - 1, current_pos + delta))
    ...
    def run(self) -> int:
        ...
        try:
            ...
            # curses 실패 시 폴백: 기존 번호 입력 방식
            print(colored_text(f"\n⚠️ 화살표 메뉴 초기화 실패, 번호 입력 모드로 전환: {e}", Colors.WARNING))
            return self._fallback_menu()

    def _fallback_menu(self) -> int:
        """curses 실패 시 폴백 메뉴"""
        if self.title:
            print(colored_text(f"\n{self.title}", Colors.HEADER))
            print("-" * 40)
...
def interactive_select(
    ...
    """
    menu = InteractiveMenu(items, title=title, footer=footer, show_index=show_index)
    return menu.run()


# ----------------------------------------------------------------------------
# 메뉴 입력 검증 헬퍼 (v5.4.0 신규)
# ----------------------------------------------------------------------------
def get_menu_choice(
    prompt: str,
    max_num: int,
...
class FileTransferManager:
    ...
    def download_file_from_s3_to_ec2(self, s3_key: str, remote_path: str, instance_id: str, instance_name: str) -> FileTransferResult:
        ...
        try:
            ...
            # S3에서 EC2로 다운로드 명령 (v5.4.0: shlex.quote 적용으로 인젝션 방지)
            ...
            if Config.DEBUG_MODE:
                ...
                # 프로덕션 모드: 간결한 출력 (v5.4.0: shlex.quote 적용)
                command = f"""
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_concurrent) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_instance):
                try:
                    ...
                    # v5.4.0: 에러 발생 시에도 결과에 추가하여 전체 상태 추적 가능하도록 개선
                    instance = future_to_instance[future]
...
def get_db_credentials(db_user_hint=""):
    ...
    # 4. 자격 증명 저장 여부 확인 (v5.4.0: Keychain 옵션 추가)
...
def eks_menu(manager: AWSManager, region: str):
    ...
    while True:
        ...
        title = f"EKS Clusters  │  Region: {region}"
...
def ecs_menu(manager: AWSManager, region: str):
    ...
    while True:
        ...
        title = f"ECS Clusters  │  Region: {region}"
        cluster_sel = interactive_select(cluster_items, title=title)
...
╠══════════════════════════════════════════════════════════════════════════════╣
║ v5.4.0 주요 변경사항                                                          ║
║   - macOS Keychain 연동 (DB 패스워드 안전 저장)                                  ║
║   - 리소스별 캐시 TTL 차등 적용                                                  ║
║   - 페이지네이션 지원 (대량 결과 표시)                                           ║
║   - 명령줄 옵션 확장 (-s, --no-cache)                                          ║
║   - shlex.quote() 적용 (보안 강화)                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
""",
    'ec2': """
╔══════════════════════════════════════════════════════════════════════════════╗
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='AWS EC2/RDS/ElastiCache/ECS/EKS 연결 도구 v5.4.0 (macOS)',
...
'''
    ...
    parser.add_argument('-s', '--service', choices=['ec2', 'rds', 'cache', 'ecs', 'eks'],
                        help='직접 진입할 서비스 (v5.4.0)')
    parser.add_argument('--no-cache', action='store_true', help='캐시 비활성화 (v5.4.0)')
    parser.add_argument('-v', '--version', action='version', version='%(prog)s v5.4.0')
    args = parser.parse_args()

    # 캐시 비활성화 옵션 처리 (v5.4.0)
    if args.no_cache:
        ...
        while True:
            ...
            while True:
                ...
                if selected == -1 or selected == len(menu_items) - 1:  # 취소 또는 종료
                    ...
                    show_main_help()
                elif has_creds and selected == 8:  # 자격증명 삭제 (자격증명 있을 때)
                    clear_stored_credentials()
                elif (has_creds and selected == 9) or (not has_creds and selected == 8):  # 리전 재선택
                    break
