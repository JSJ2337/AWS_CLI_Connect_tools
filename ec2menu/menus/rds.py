...
def connect_to_rds(manager: AWSManager, tool_path: str, region: str) -> None:
    while True:
        ...
        db = dbs[sel]
        target_region = db.get('_region', region)
        ...
        procs = []
        try:
            print(colored_text(f"🔹 포트 포워딩: [localhost:{local_port}] -> [{db['Id']}:{db['Port']}] ({target_region})", Colors.INFO))
            add_to_history('rds', manager.profile, target_region, db['Id'], db['Id'])
            params_dict = {
                "host": [db["Endpoint"]],
            ...
            print(colored_text("\n✅ 포트 포워딩 활성화. DBeaver로 자동 연결합니다...", Colors.SUCCESS))

            dbeaver_path = Config.DBEAVER_PATH
            if Path(dbeaver_path).exists():
                ...
                # mysql 비밀번호는 MYSQL_PWD 환경변수로 전달해 `ps` 노출 방지
                command = [tool_path, '-h', 'localhost', '-P', str(local_port), '-u', db_user]
                if db.get('DBName'):
